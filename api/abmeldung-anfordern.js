...
const { insertRequest, sha256 } = require('../lib/supabase');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!address) {
      return json(res, 200, { message: 'Wenn die Adresse bei uns hinterlegt ist, erhältst du gleich eine E-Mail mit dem Bestätigungslink.' });
    }
    ...
    await insertRequest({
      ...
      address_name: [firstName, address.lastName].filter(Boolean).join(' '),
      status: 'pending',
    ...
    console.error('abmeldung-anfordern', error);
    return json(res, 500, { message: 'Die Anfrage konnte gerade nicht verarbeitet werden.' });
  }
};